...
function FogliAssistenzaListPage({ session, loadingAnagrafiche, clienti: allClienti, tecnici: allTecnici, commesse: allCommesse, ordini: allOrdini }) {
    ...
    const fetchFogliDaServer = useCallback(async () => {
        ...
        if (!session || loadingAnagrafiche) {
            // console.log("Fetch fogli in attesa: sessione o anagrafiche non pronte.");
        ...
        if (filtroDataDa) {
            query = query.gte('data_apertura_foglio', filtroDataDa);
        }
        if (filtroDataA) {
    ...
    }, [fogli, filtroClienteTesto, filtroCommessaTesto, filtroOrdineTesto, filtroTecnicoTesto]);
